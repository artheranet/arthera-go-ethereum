...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"art": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + art.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + art.blockNumber + " (" + new Date(1000 * art.getBlock(art.blockNumber).timestamp) + ")\n";
		try {
