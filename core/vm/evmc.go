...
func (host *hostContext) AccessAccount(addr evmc.Address) evmc.AccessStatus {
	...
	if !conf.IsBerlin(host.env.Context.BlockNumber) {
...
func (host *hostContext) setStorageEIP2200(original, current, value *uint256.Int) (status evmc.StorageStatus) {
	...
	if original == current {
		...
		if value.IsZero() {
			...
			host.env.StateDB.AddRefund(params.SstoreClearsScheduleRefundEIP2200)
	...
	if !original.IsZero() {
		...
		if current.IsZero() {
			...
			host.env.StateDB.SubRefund(params.SstoreClearsScheduleRefundEIP2200)
	...
	if original.Eq(value) {
		...
		if original.IsZero() {
			...
			// 20000 - 800
			host.env.StateDB.AddRefund(params.SstoreSetGasEIP2200 - params.SloadGasEIP2200)
		...
		// 3.2.2.2. "Otherwise"
		{
			...
			host.env.StateDB.AddRefund(params.SstoreResetGasEIP2200 - params.SloadGasEIP2200)
...
func (host *hostContext) SetStorage(evmcAddr evmc.Address, evmcKey evmc.Hash, evmcValue evmc.Hash) (status evmc.StorageStatus) {
	...
	conf := host.env.ChainConfig()
	if conf.IsIstanbul(host.env.Context.BlockNumber) {
		return host.setStorageEIP2200(original, current, value)
	} else if conf.IsConstantinople(host.env.Context.BlockNumber) {
...
func (host *hostContext) GetTxContext() evmc.TxContext {
	...
	if conf.IsLondon(host.env.Context.BlockNumber) {
		txCtx.BaseFee = evmc.Hash(common.BigToHash(host.env.Context.BaseFee))
		// TODO: Arthera - the RANDOM opcode after merge ?
...
func (host *hostContext) Call(kind evmc.CallKind,
	...
	case evmc.Create:
		...
		conf := host.env.ChainConfig()
		isHomestead := conf.IsHomestead(host.env.Context.BlockNumber)
